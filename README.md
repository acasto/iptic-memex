...
- Useful keys:
  ...
  - Aspect toggles: `log_tool_use`, `log_cmd`, `log_provider`, `log_settings`, `log_errors`, `log_usage`, `log_messages`, `log_mcp`, `log_rag`, `log_web`
...
- Args: `content` and `personas` are required; optional `goal`, `notes`, `artifact_type`, `panel`, `files`, and `model`. `personas` may be names (CSV/list) or a path to a Markdown personas file.
