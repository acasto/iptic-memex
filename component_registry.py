...
class PromptResolver:
    ...
    def _resolve_single_prompt(self, prompt_name: str) -> Optional[str]:
        ...
        if user_prompt_dir:
            user_dir = self.config.resolve_directory_path(user_prompt_dir)
            if user_dir:
        ...
        if prompt_dir:
            default_dir = self.config.resolve_directory_path(prompt_dir)
            if default_dir:
    ...
    def _load_prompt_from_directory(self, prompt_name: str, directory: str) -> Optional[str]:
...
class ComponentRegistry:
    ...
    def create_action_with_bridge(self, name: str, session_bridge):
        ...
        try:
            ...
            if user_actions_dir:
                user_dir = self.config.resolve_directory_path(user_actions_dir)
                if user_dir:
    ...
    def _try_load_action_from_directory(self, name: str, directory: str, session_bridge):
    ...
    def _try_load_from_directory(self, name: str, directory: str):
