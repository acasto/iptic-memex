...
def test_demo_discover_register_and_specs():
    ...
    # Build tool specs; ensure dynamic tools are present
    act = AssistantCommandsAction(sess)
    specs = {s['name']: s for s in act.get_tool_specs()}
    assert 'mcp:testmcp/calc.sum' in specs
    assert 'mcp:testmcp/echo.say' in specs
    # Ensure parameters reflect schema
    calc_props = specs['mcp:testmcp/calc.sum']['parameters']['properties']
    assert 'a' in calc_props and 'b' in calc_props
    echo_props = specs['mcp:testmcp/echo.say']['parameters']['properties']
