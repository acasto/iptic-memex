...
class LlamaCppProvider(APIProvider):
    ...
    def __init__(self, session: SessionHandler):
        ...
        if 'api_key' in self.params and self.params['api_key'] is not None:
            ...
            self.api_key = 'none'  # it seems the openai library requires an api key, so we'll set it to none for now

        # todo: should we keep base_url or derive it from endpoint?
    ...
    def assemble_message(self) -> list:
        ...
        """
        message = []
        if self.session.get_context('prompt') and self.session.get_context('prompt').get()['name'] != 'none':
